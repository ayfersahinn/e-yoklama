...
class _ProfilimState extends State<Profilim> {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Öğrenci Profil", style: TextStyle(fontSize: 20, color: Colors.white,fontFamily: "Comfortaa",fontWeight: FontWeight.bold),),
          centerTitle: true,
  ...
  void baglantiAl() async
  {
    String baglanti = await FirebaseStorage.instance
        ...
        .child("Pictures")
        .child(kullanici!.id)
        .child("profile").getDownloadURL();
    setState((){
      indirmeBaglantisi=baglanti;
    });
  }
  void resimYukle(ImageSource source)async{
    final dbRef = await FirebaseFirestore.instance.collection("users").doc().collection("photos");
